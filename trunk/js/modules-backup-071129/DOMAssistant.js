...
var DOMAssistant = {
	...
	init : function (){
		...
		var current;
		for(var i=0, il=this.baseMethodsToAdd.length; i<il; i++){
	...
	addMethods : function (elm){
		if(elm){
			...
			for(var i=0, il=elms.length; i<il; i++){	
				for(var j=0, jl=this.methodsToAdd.length; j<jl; j++){
				...
				for(var i=0, il=cssSelectors.length; i<il; i++){
					...
					if(childOrSiblingRef){
						...
						if(attributeMatch){
							...
							for(var o=0, ol=attributeSelectors.length; o<ol; o++){
								attributeMatch = /(\w+)?\[(\w+)=?(\w+)?\]/.exec(attributeSelectors[o]);
	...
	elmsByAttribute : function (attr, attrVal, tag){
		if(document.evaluate){
			...
			DOMAssistant.elmsByAttribute = function (attr, attrVal, tag){
			    ...
			    for(var i=0,current,currentAttr,il=elms.length; i<il; i++){
			        current = elms[i];
